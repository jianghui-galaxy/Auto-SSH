...
2、生成ssh key的时候需要连续输入三次回车，有交互，解决办法：利用`-P`选项指定空密码，同时利用`-f`选项指定生成的文件路径和名称，并且用`1>/dev/null`将标准输出屏蔽（不屏蔽标准错误）
...
1、给定一个或多个主机IP，放到`hosts.all`文件中，所有主机的 `$USER` 用户的密码一致并且为 `$PASSWORD`，现在需要配置这些主机任意两两之间的ssh无密登录（多到多双向）
